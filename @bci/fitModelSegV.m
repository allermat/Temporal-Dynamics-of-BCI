function [negLogLike,mdlEval] = fitModelSegV(parameters,parameterNames,dataVA,responseLoc)
...
% USAGE:
%   [logLike,all] = bci.fitModelSegV(parameters,parameterNames,dataVA,responseLoc)
% INPUT:
...
if any(cellfun(@isempty,{muP,sigP,sigV,kernelWidth}))
...
for indCond = 1:nCond
    ...
    sV_hat_indep = (xV/varV + muP/varP) * varV_hat;
    ...
    if length(responseLoc) > 1
        ...
        [~,tV] = min(abs(repmat(sV_hat_indep,1,lengthRespLoc) - repmat(responseLoc,nIntSamples,1)),[],2);
        [~,tA] = min(abs(repmat(sV_hat_indep,1,lengthRespLoc) - repmat(responseLoc,nIntSamples,1)),[],2);
        sV_pred_resp = responseLoc(tV);
        sA_pred_resp = responseLoc(tA);
    else
        %continuous responses used, no discretisation
        sV_pred_resp = sV_hat_indep;
        sA_pred_resp = sV_hat_indep;
    end
